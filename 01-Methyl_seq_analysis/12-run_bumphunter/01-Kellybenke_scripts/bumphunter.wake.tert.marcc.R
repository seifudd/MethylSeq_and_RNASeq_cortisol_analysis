...
wake.values=c(pheno1$meanNCortW,pheno2$meanNCortW)
...
qf=as.numeric(qcut(wake.values,3))
...
#Wake
setwd("/data/NHLBI_BCB/Fayaz/19-rales12-bumphunter-results-dichot")
load("rales12.wake.sva.Rdata")
sv=as.data.frame(wake.sva$sv)
colnames(sv)=paste("sv",1:ncol(sv),sep="")

beta2 = beta2[,which(wake.tert!="NA")]

pheno_mat = model.matrix(~as.numeric(wake.tert) + as.numeric(age) + as.factor(sex) + as.matrix(sv))
...
setwd("/data/NHLBI_BCB/Fayaz/19-rales12-bumphunter-results-dichot")
write.table(r12.bump$table, file="rales12.bump.wake.tertile.txt")
rales12.bump.pvaluesMarginal = data.frame(cbind(beta=r12.bump$coef, pvaluesMarginal=r12.bump$pvaluesMarginal))
write.table(rales12.bump.pvaluesMarginal, file="rales12.bump.pvalues.wake.tertiles.txt")
