...
stress.values=c(pheno1$meanNEstress,pheno2$meanNEstress)
...
qf=as.numeric(qcut(stress.values,3))
...
#Wake
setwd("/data/NHLBI_BCB/Fayaz/19-rales12-bumphunter-results-dichot")
load("rales12.stress.sva.Rdata")
sv=as.data.frame(stress.sva$sv)
colnames(sv)=paste("sv",1:ncol(sv),sep="")

beta2 = beta2[,which(stress.tert!="NA")]

pheno_mat = model.matrix(~as.numeric(stress.tert) + as.numeric(age) + as.factor(sex) + as.matrix(sv))
...
setwd("/data/NHLBI_BCB/Fayaz/19-rales12-bumphunter-results-dichot")
write.table(r12.bump$table, file="rales12.bump.stress.tertiles.txt")
rales12.bump.pvaluesMarginal = data.frame(cbind(beta=r12.bump$coef, pvaluesMarginal=r12.bump$pvaluesMarginal))
write.table(rales12.bump.pvaluesMarginal, file="rales12.bump.pvalues.stress.tertiles.txt")
